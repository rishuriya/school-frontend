  ...
  const navigation: Array<{ name: string; href?: string; children?: { name: string; href: string }[] }> = [
    ...
    },
    {
      ...
      children: [
        ...
        { name: 'Syllabus', href: '/students/syllabus' },
    ...
    },
    {
      ...
      children: [
        ...
        { name: 'Transfer Certificate', href: '/students/transfer-certificate' },
            ...
            {navigationWithFacilities.map((item) => (
              ...
              >
                {item.href ? (
                  <Link
                    href={item.href}
                    className={`${overlay ? 'text-white hover:text-white/90' : 'text-gray-900 hover:text-blue-600'} px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm font-semibold transition-all duration-300 hover:scale-105 inline-flex items-center rounded-xl hover:bg-white/10 backdrop-blur-sm whitespace-nowrap`}
                  >
                    {item.name}
                    {item.children && (
                  ...
                  <span className={`${overlay ? 'text-white' : 'text-gray-900'} px-2 lg:px-4 py-2 lg:py-3 text-xs lg:text-sm font-semibold whitespace-nowrap`}>{item.name}</span>
                )}
                          ...
                          <div className="flex items-center">
                            <span className="group-hover/item:scale-110 transition-transform duration-200">
                              {child.name.includes('Curriculum') ? '📚' :
                                child.name.includes('Syllabus') ? '📑' :
                                  child.name.includes('Transfer Certificate') ? '📜' :
