export const APP_CONFIG = {
  // API Configuration
  api: {
    baseUrl: process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:3000/api',
    useMockData: process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true' || false, // Use real data by default
    timeout: 10000, // 10 seconds
  },
  
  // School Information
  school: {
    id: process.env.NEXT_PUBLIC_SCHOOL_ID || '68c22a22ec3c0fd06634bc93', // School ID from backend
    name: 'Bright Future Academy',
    tagline: 'Empowering Minds, Building Futures',
    established: 1995,
    address: '123 Education Street, Knowledge City, KC 12345',
    phone: '+1 (555) 123-4567',
    email: 'info@brightfutureacademy.edu',
    website: 'www.brightfutureacademy.edu',
  },
  
  // Features Configuration
  features: {
    enableNews: true,
    enableEvents: true,
    enableFaculty: true,
    enableStudents: true,
    enablePrograms: true,
    enableContactForm: true,
    enableSocialMedia: true,
  },
  
  // UI Configuration - Elegant Color Palette
  ui: {
    theme: {
      // Primary Colors
      primary: {
        50: '#eff6ff',
        100: '#dbeafe',
        200: '#bfdbfe',
        300: '#93c5fd',
        400: '#60a5fa',
        500: '#3b82f6',
        600: '#2563eb',
        700: '#1d4ed8',
        800: '#1e40af',
        900: '#1e3a8a',
      },
      // Secondary Colors
      secondary: {
        50: '#f8fafc',
        100: '#f1f5f9',
        200: '#e2e8f0',
        300: '#cbd5e1',
        400: '#94a3b8',
        500: '#64748b',
        600: '#475569',
        700: '#334155',
        800: '#1e293b',
        900: '#0f172a',
      },
      // Accent Colors
      accent: {
        50: '#fefce8',
        100: '#fef9c3',
        200: '#fef08a',
        300: '#fde047',
        400: '#facc15',
        500: '#eab308',
        600: '#ca8a04',
        700: '#a16207',
        800: '#854d0e',
        900: '#713f12',
      },
      // Success Colors
      success: {
        50: '#f0fdf4',
        100: '#dcfce7',
        200: '#bbf7d0',
        300: '#86efac',
        400: '#4ade80',
        500: '#22c55e',
        600: '#16a34a',
        700: '#15803d',
        800: '#166534',
        900: '#14532d',
      },
      // Warning Colors
      warning: {
        50: '#fffbeb',
        100: '#fef3c7',
        200: '#fde68a',
        300: '#fcd34d',
        400: '#fbbf24',
        500: '#f59e0b',
        600: '#d97706',
        700: '#b45309',
        800: '#92400e',
        900: '#78350f',
      },
      // Error Colors
      error: {
        50: '#fef2f2',
        100: '#fee2e2',
        200: '#fecaca',
        300: '#fca5a5',
        400: '#f87171',
        500: '#ef4444',
        600: '#dc2626',
        700: '#b91c1c',
        800: '#991b1b',
        900: '#7f1d1d',
      },
      // Gradients
      gradients: {
        primary: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
        secondary: 'linear-gradient(135deg, #f093fb 0%, #f5576c 100%)',
        accent: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)',
        success: 'linear-gradient(135deg, #43e97b 0%, #38f9d7 100%)',
        warning: 'linear-gradient(135deg, #fa709a 0%, #fee140 100%)',
        elegant: 'linear-gradient(135deg, #667eea 0%, #764ba2 50%, #f093fb 100%)',
        modern: 'linear-gradient(135deg, #a8edea 0%, #fed6e3 100%)',
        corporate: 'linear-gradient(135deg, #2c3e50 0%, #3498db 100%)',
      },
    },
    layout: {
      maxWidth: '1280px', // max-w-7xl
      containerPadding: '1rem', // px-4
    },
  },
  
  // SEO Configuration
  seo: {
    defaultTitle: 'Bright Future Academy - Empowering Minds, Building Futures',
    defaultDescription: 'A premier educational institution committed to academic excellence, character development, and innovative learning.',
    siteName: 'Bright Future Academy',
    siteUrl: process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000',
  },
};

// Helper function to check if mock data should be used
export const isUsingMockData = (): boolean => {
  return APP_CONFIG.api.useMockData;
};

// Helper function to get API base URL
export const getApiBaseUrl = (): string => {
  return APP_CONFIG.api.baseUrl;
};

// Helper function to get full API URL
export const getApiUrl = (endpoint: string): string => {
  return `${getApiBaseUrl()}${endpoint}`;
}; 